...
vec3 thresholdFilter(vec3 color)
{
    ...
    soft = soft * soft * 0.5; // quadratic falloff
...
void main()
{
    ...
    vec3 a = texture(u_SourceTex, uv + vec2(-2.0, -2.0) * texelSize).rgb;
    vec3 b = texture(u_SourceTex, uv + vec2( 0.0, -2.0) * texelSize).rgb;
    vec3 c = texture(u_SourceTex, uv + vec2( 2.0, -2.0) * texelSize).rgb;
    vec3 d = texture(u_SourceTex, uv + vec2(-2.0,  0.0) * texelSize).rgb;
    vec3 e = texture(u_SourceTex, uv).rgb;
    vec3 f = texture(u_SourceTex, uv + vec2( 2.0,  0.0) * texelSize).rgb;
    vec3 g = texture(u_SourceTex, uv + vec2(-2.0,  2.0) * texelSize).rgb;
    vec3 h = texture(u_SourceTex, uv + vec2( 0.0,  2.0) * texelSize).rgb;
    vec3 i = texture(u_SourceTex, uv + vec2( 2.0,  2.0) * texelSize).rgb;

    // 额外 4 个对角半像素采样
    vec3 j = texture(u_SourceTex, uv + vec2(-1.0, -1.0) * texelSize).rgb;
    vec3 k = texture(u_SourceTex, uv + vec2( 1.0, -1.0) * texelSize).rgb;
    vec3 l = texture(u_SourceTex, uv + vec2(-1.0,  1.0) * texelSize).rgb;
    vec3 m = texture(u_SourceTex, uv + vec2( 1.0,  1.0) * texelSize).rgb;
